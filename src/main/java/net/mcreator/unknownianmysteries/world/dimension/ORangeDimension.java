...
import net.mcreator.unknownianmysteries.item.ORangeItem;
import net.mcreator.unknownianmysteries.block.UnknownianRedPortalBlockBlock;
import net.mcreator.unknownianmysteries.block.ORangeStoneBlock;
...
public class ORangeDimension extends UnknownianmysteriesModElements.ModElement {
	@ObjectHolder("unknownianmysteries:o_range_portal")
	public static final CustomPortalBlock portal = null;

	public ORangeDimension(UnknownianmysteriesModElements instance) {
		super(instance, 162);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(ORangeStoneBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:o_range_forest")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:o_range_forest")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:o_range_plains")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:o_range_plains")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("unknownianmysteries:o_range"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("o_range_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("o_range_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("o_range_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new ORangeItem().setRegistryName("o_range"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("unknownianmysteries:o_range"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("unknownianmysteries:o_range")));
				} else {
					entity.func_242279_ag();
